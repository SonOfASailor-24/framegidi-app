...
// ---------------- HANDOVER (real shared data — live snapshot + persisted log) ----------------
// ---------------- REWORK (fully independent from Framed/Canvas/TBR jobs) ----------------
// ---------------- SHARED: reuse existing job data, never re-enter it ----------------
...
function JobInfoCard({ job }) {
  const types = getJobTypes(job);
  const typeLabels = [types.framed && "Framed", types.canvas && "Canvas", types.tbr && "TBR"].filter(Boolean).join(" + ");
  return (
    <div className="mb-4 rounded-md border border-neutral-800 bg-neutral-900 p-3 text-sm">
      ...
      <div className="mt-1 text-xs text-neutral-500">{typeLabels || "—"} · Total {job.quantity} pcs</div>
      {types.framed && (
...
// ---------------- JOB TRACKING (productivity — separate from the APC audit trail) ----------------
function JobTrackingModule({ jobs, apcName, cutRecords, coupleRecords, onCreateCut, onCreateCouple, editable }) {
  ...
  return (
    <div>
      ...
      {!job && <EmptyState text="Select a job to record cutting or coupling." />}
      {job && (
        <>
          <JobInfoCard job={job} />
...
function CuttingForm({ job, apcName, onCreate }) {
  ...
  function submit() {
    ...
    onCreate({ id: newId("cut"), jobId: job.id, jobRef: job.jobRef || job.clientName, clientName: job.clientName,
      material, size: size.trim(), qty, cutBy: cutBy.trim(), notes: notes.trim(), recordedBy: apcName, createdAt: Date.now() });
...
function CouplingForm({ job, apcName, onCreate }) {
  ...
  function submit() {
    ...
    onCreate({ id: newId("couple"), jobId: job.id, jobRef: job.jobRef || job.clientName, clientName: job.clientName,
      size: size.trim(), qty, coupledBy: coupledBy.trim(), notes: notes.trim(), recordedBy: apcName, createdAt: Date.now() });
      ...
      <div className="space-y-1.5">
        ...
        {all.map((r, i) => (
          <div key={i} className="rounded-md border border-neutral-800 bg-neutral-900 p-2.5 text-xs">
            ...
            <div className="mt-1 text-neutral-400">{r.size} × {r.qty} · {r.kind === "Cutting" ? "Cut" : "Coupled"} by: {r.by}</div>
...
function InstallForm({ job, apcName, existing, onSave }) {
  ...
  return (
    <div className="mb-4 space-y-2 rounded-md border border-neutral-800 bg-neutral-950/40 p-3">
      <Field label="Installer(s)"><input value={installers} onChange={(e) => setInstallers(e.target.value)} placeholder="e.g. Dare, Ope" className="input" /></Field>
      <div className="grid grid-cols-2 gap-2">
...
function InstallSummary({ rec }) {
  return (
    <div className="rounded-md border border-neutral-800 bg-neutral-900 p-2.5 text-xs">
      ...
      <div className="mt-1 text-neutral-400">{rec.date || "—"} {rec.time || ""} · {rec.location || "—"}</div>
